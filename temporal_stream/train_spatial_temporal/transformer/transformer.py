...
class Transformer(nn.Module):
    ...
    def forward(self, x, mask = None):
        ...
        x = torch.cat((cls_tokens, x), dim=1) # [b, frame+cls, d]
        x += self.pos_embedding[:, :(n + 1)]

        x = self.dropout(x)

        for attn, ff in self.layers:
            # print("forward in the tr.")
            x = attn(x)
            x = ff(x)
        ...
        # Identity
        x = self.to_latent(x) # [B x 1 (from CLS) x d]
