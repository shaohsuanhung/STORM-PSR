...
def build_random_oversampler(PSR_labels, frame_per_seg, skip_factor, num_frames, mode='bimodal'):
    ...
    #--------- Parameters for different temporal receptive field
    ROI, epsilon, sep, VIP_pos, VIP_w, IP_pos, IP_w, other, other_w = find_sampling_params(mode, frame_per_seg)
    # print(f"Sanity check of the hyper-para of KCA, ROI:{ROI}, sep:{sep}")
...
def find_sampling_params(mode: str, frame_per_seg: int):
    """ Assign different ROI, and sep hyperparameters to different temporal receptive field.
    """
    if (mode == 'bimodal'):
        if frame_per_seg == 4:
            ROI = 0.25
        ...
        if frame_per_seg == 32:
            ...
            epsilon = 1e-8
            sep = 9 
            VIP_pos = [5,6,10,11,12,13,14,15,16]
            IP_pos = [3,4]
            other = [0,1,2,7,8,9]
            VIP_w = 1000
            IP_w  = 1000
            ...
            print(f"The value of frame_per_seg {frame_per_seg} is not seen in the setting. Use the default setting")
            ROI = 0.25
            epsilon = 1e-8
            sep = 1 
            VIP_pos = [5,6,10,11,12,13,14,15,16]
        ...
        if frame_per_seg == 32:
            ...
            print("The value of frame_per_seg is not seen in the setting. Use the default setting")
            ROI = 1
            epsilon = 1e-8
            VIP_pos = [5,6,10,11,12,13,14,15,16]
            IP_pos = [3,4]
        ...
        raise NotImplementedError(f"Expect 'gaussian' or 'bimodal', but get {mode}.")
    
    return ROI, epsilon, sep, VIP_pos, VIP_w, IP_pos, IP_w, other, other_w
...
if __name__ == '__main__':
    #- Test the EmbeddingRecord_PSR object. Check whether it load the correct labels.
    import utils
    root_dir = Path('/shared/nl011006/res_ds_ml_restricted/shaohung/MECCANO-PSR')
    embedding_csv_dir = Path('/shared/nl011006/res_ds_ml_restricted/shaohung/train_log/runs_MECCANO/embeddings_dataset/test')
    test_label = None
    testset_df = utils.load_embedding_and_label_df( embedding_csv_dir, test_label, statics=False, normalized=False)
    embedding_list = [EmbeddingRecord_PSR(testset_df[testset_df['filename'] == name],
                                          root_datapath=root_dir / 'test',
                                          name=name,
                                          FRAMES_PER_SEGMENT=65,
                                          sampling_strategy='gaussian',
                                          execution_mode='no_error',
                                          skip_factor = 3,
                                          num_dig_psr = 17,
                                          test_mode=True) for name in ['0010']]
    for clip_idx, recording in enumerate(embedding_list):
        for embedding,frameID, label in (zip(recording.clipped_embeddings, recording.clipped_last_frameID,recording.clipped_level_labels)):
            print(f'Frame:{frameID},{label}')

    # %% 
    #-- Test the videoRecord and VideoDataset in test mode
    img_path = Path(
        '/shared/nl011006/res_ds_ml_restricted/TimSchoonbeek/meccano/frames/test')
    psr_path = Path(
        '/shared/nl011006/res_ds_ml_restricted/shaohung/MECCANO-PSR/test')

    video_list = [VideoRecord(img_datapath=img_path / name, psr_datapath=psr_path,
                              name=name, num_dig_psr=17, frame_per_seg=64, skip_factor=5,
                              sampling_strategy='gaussian', execution_mode='no_error', test_mode=True)
                  for name in ['0008']]

    for clip_idx, recording in enumerate(video_list):
        for idx, (frame, label, frameID) in enumerate(zip(recording.clipped_embeddings, recording.clipped_level_labels, recording.clipped_last_frameID)):
            print(f'Frame:{Path(frame[-1]).name},{label}')
    
    # %% 
    #-- Test the dataloader of the EmbeddingFrameDataset_PSR object
    from torch.utils.data import DataLoader
    from tqdm import tqdm
    from sklearn.metrics import classification_report

    img_path = Path('/shared/nl011006/res_ds_ml_restricted/TimSchoonbeek/meccano/frames')
    psr_path = Path('/shared/nl011006/res_ds_ml_restricted/shaohung/MECCANO')


    train_set = EmbeddingFrameDataset_PSR(
        root_datapath= Path('/shared/nl011006/res_ds_ml_restricted/shaohung/train_log/runs_MECCANO/embeddings_dataset/train'),
        psr_root_dir=Path('/shared/nl011006/res_ds_ml_restricted/shaohung/MECCANO-PSR/train'),
        split='train',
        num_frame_per_seg=65,
        transform=None,
        test_mode=False,
    ...
    train_loader = DataLoader(train_set, batch_size=1,shuffle=True, num_workers=16, persistent_workers=True, pin_memory=True)
    ...
    for epoch in range(1, 2):
        ...
        for i, (src, target) in progress:
            ...
            for one_label in target:
                for ele in one_label:
                    if (ele > 1):
                        print(f"Problematic label{target}")

            y_pred.extend(target)
            y_true.extend(target)

    #- Check the number of instance that being sampled in the __getitem__
    print(classification_report(
        np.array(y_true),
        np.array(y_pred),
        output_dict=False,
        zero_division=np.nan))
    print( f"In epoch:{epoch}, get {num_non_zero}/{num_samples} meaningful samples.")

    #%% 
    #- Test the VideoFrameDataset_PSR
    from torch.utils.data import DataLoader
    img_path = Path('/shared/nl011006/res_ds_ml_restricted/TimSchoonbeek/IndustReal/recordings')
    psr_path = Path('/shared/nl011006/res_ds_ml_restricted/shaohung/IndustReal_corrected_PSR')
    ...
    train_set = VideoFrameDataset_PSR(
        img_root_path=img_path,
        psr_root_path=psr_path,
        split='train',
        num_frame_per_seg=65,
    ...
    embed_train_loader = DataLoader(train_set, batch_size=1,
                              shuffle=True, num_workers=8, persistent_workers=True, pin_memory=True)
    val_set = VideoFrameDataset_PSR(
        img_root_path=img_path,
        psr_root_path=psr_path,
        split='val',
        num_frame_per_seg=65,
    ...
    embed_val_loader = DataLoader(val_set, batch_size=1,
                              shuffle=True, num_workers=8, persistent_workers=True, pin_memory=True)
